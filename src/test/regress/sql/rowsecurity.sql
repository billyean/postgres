...
SET row_security TO ON;
COPY copy_t FROM STDIN; --ok (RLS allows a % 2 = 0)
2	bcd
4	def
\.
COPY copy_t FROM STDIN; --fail - RLS violation
1	abc
\.
...
--
-- COPY FROM with RLS (Patch 1: plain heap tables only)
--
RESET SESSION AUTHORIZATION;
SET search_path = regress_rls_schema, public;

-- Setup: create test table
CREATE TABLE copy_rls_test (id int, tenant_id int, data text);
ALTER TABLE copy_rls_test ENABLE ROW LEVEL SECURITY;
CREATE POLICY p_insert ON copy_rls_test FOR INSERT
  WITH CHECK (tenant_id = 1);
GRANT INSERT ON copy_rls_test TO regress_rls_bob;

-- T1: Row satisfies policy -> COPY succeeds
SET SESSION AUTHORIZATION regress_rls_bob;
COPY copy_rls_test FROM stdin;
1	1	hello
\.
RESET SESSION AUTHORIZATION;
...
-- T2: Row violates policy -> COPY fails with RLS error
SET SESSION AUTHORIZATION regress_rls_bob;
COPY copy_rls_test FROM stdin;
2	2	world
\.
...
-- T3: Multiple permissive policies (OR semantics)
RESET SESSION AUTHORIZATION;
CREATE POLICY p_insert2 ON copy_rls_test FOR INSERT
...
SELECT * FROM copy_rls_test;
TRUNCATE copy_rls_test;
DROP POLICY p_insert2 ON copy_rls_test;

-- T4: Restrictive policy (AND semantics)
RESET SESSION AUTHORIZATION;
CREATE POLICY p_restrict ON copy_rls_test AS RESTRICTIVE FOR INSERT
  WITH CHECK (id > 0);
SET SESSION AUTHORIZATION regress_rls_bob;
-- This should fail: restrictive policy requires id > 0
COPY copy_rls_test FROM stdin;
-1	1	negative_id
\.
-- This should succeed: both policies satisfied
COPY copy_rls_test FROM stdin;
1	1	ok
\.
RESET SESSION AUTHORIZATION;
SELECT * FROM copy_rls_test;
TRUNCATE copy_rls_test;
DROP POLICY p_restrict ON copy_rls_test;

-- T5: FORCE ROW LEVEL SECURITY makes non-superuser owner subject to policy
-- regress_rls_alice owns the table; she's not a superuser
RESET SESSION AUTHORIZATION;
CREATE TABLE copy_rls_force_test (id int, tenant_id int, data text);
ALTER TABLE copy_rls_force_test OWNER TO regress_rls_alice;
ALTER TABLE copy_rls_force_test ENABLE ROW LEVEL SECURITY;
CREATE POLICY p_insert ON copy_rls_force_test FOR INSERT
  WITH CHECK (tenant_id = 1);
ALTER TABLE copy_rls_force_test FORCE ROW LEVEL SECURITY;
GRANT INSERT ON copy_rls_force_test TO regress_rls_alice;

SET SESSION AUTHORIZATION regress_rls_alice;
COPY copy_rls_force_test FROM stdin;
11	99	owner_blocked
\.
RESET SESSION AUTHORIZATION;
SELECT * FROM copy_rls_force_test;
TRUNCATE copy_rls_force_test;

-- T6: Owner without FORCE RLS bypasses policy
ALTER TABLE copy_rls_force_test NO FORCE ROW LEVEL SECURITY;
SET SESSION AUTHORIZATION regress_rls_alice;
COPY copy_rls_force_test FROM stdin;
10	99	owner_bypass
\.
RESET SESSION AUTHORIZATION;
SELECT * FROM copy_rls_force_test;
DROP TABLE copy_rls_force_test;

-- T7: BEFORE INSERT trigger modifies row from failing to passing
CREATE FUNCTION copy_rls_fix_tenant() RETURNS trigger AS $$
BEGIN
  NEW.tenant_id := 1;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER tr_fix BEFORE INSERT ON copy_rls_test
  FOR EACH ROW EXECUTE FUNCTION copy_rls_fix_tenant();
...
-- T8: BEFORE INSERT trigger modifies row from passing to failing
CREATE OR REPLACE FUNCTION copy_rls_fix_tenant() RETURNS trigger AS $$
BEGIN
  NEW.tenant_id := 99;
  RETURN NEW;
END;
...
SELECT * FROM copy_rls_test;
TRUNCATE copy_rls_test;
DROP TRIGGER tr_fix ON copy_rls_test;
DROP FUNCTION copy_rls_fix_tenant();

-- T9: Stored generated column referenced by policy
CREATE TABLE copy_rls_gen_test (
  id int,
  base_val int,
  computed_val int GENERATED ALWAYS AS (base_val * 10) STORED,
  data text
);
ALTER TABLE copy_rls_gen_test ENABLE ROW LEVEL SECURITY;
CREATE POLICY p ON copy_rls_gen_test FOR INSERT
  WITH CHECK (computed_val <= 100);
GRANT INSERT ON copy_rls_gen_test TO regress_rls_bob;

SET SESSION AUTHORIZATION regress_rls_bob;
-- computed_val = 50 <= 100 -> succeeds
COPY copy_rls_gen_test (id, base_val, data) FROM stdin;
1	5	passes
\.
-- computed_val = 150 > 100 -> fails
COPY copy_rls_gen_test (id, base_val, data) FROM stdin;
2	15	fails
\.
RESET SESSION AUTHORIZATION;
SELECT * FROM copy_rls_gen_test;
DROP TABLE copy_rls_gen_test;

-- T10: Default column referenced by policy
CREATE TABLE copy_rls_default_test (
  id int,
  status text DEFAULT 'active',
  data text
);
ALTER TABLE copy_rls_default_test ENABLE ROW LEVEL SECURITY;
CREATE POLICY p ON copy_rls_default_test FOR INSERT
  WITH CHECK (status = 'active');
GRANT INSERT ON copy_rls_default_test TO regress_rls_bob;

SET SESSION AUTHORIZATION regress_rls_bob;
-- T11: COPY FROM with explicit column list; status defaults to 'active'
COPY copy_rls_default_test (id, data) FROM stdin;
1	test
\.
RESET SESSION AUTHORIZATION;
SELECT * FROM copy_rls_default_test;
DROP TABLE copy_rls_default_test;

-- T12: COPY FROM with DEFAULT token
CREATE TABLE copy_rls_deftoken (
  id int,
  tenant_id int DEFAULT 1,
  data text
);
ALTER TABLE copy_rls_deftoken ENABLE ROW LEVEL SECURITY;
CREATE POLICY p ON copy_rls_deftoken FOR INSERT
  WITH CHECK (tenant_id = 1);
GRANT INSERT ON copy_rls_deftoken TO regress_rls_bob;

SET SESSION AUTHORIZATION regress_rls_bob;
COPY copy_rls_deftoken FROM stdin WITH (default '\D');
1	\D	default_test
\.
RESET SESSION AUTHORIZATION;
SELECT * FROM copy_rls_deftoken;
DROP TABLE copy_rls_deftoken;

-- T13: CHECK constraint and RLS both fail -> RLS error wins
CREATE TABLE copy_rls_order_test (
  id int CHECK (id > 0),
  tenant_id int
);
ALTER TABLE copy_rls_order_test ENABLE ROW LEVEL SECURITY;
CREATE POLICY p ON copy_rls_order_test FOR INSERT
  WITH CHECK (tenant_id = 1);
GRANT INSERT ON copy_rls_order_test TO regress_rls_bob;

SET SESSION AUTHORIZATION regress_rls_bob;
-- Both CHECK (id > 0) and RLS (tenant_id = 1) fail; RLS error should appear
COPY copy_rls_order_test FROM stdin;
-1	99
\.
RESET SESSION AUTHORIZATION;
...
INSERT INTO allowed_depts VALUES (1), (2);
CREATE POLICY p ON copy_rls_sublink_test FOR INSERT
  WITH CHECK (dept_id IN (SELECT dept_id FROM allowed_depts));
GRANT INSERT ON copy_rls_sublink_test TO regress_rls_bob;
...
-- T16: Table without RLS behaves as before
CREATE TABLE copy_no_rls_test (id int, data text);
GRANT INSERT ON copy_no_rls_test TO regress_rls_bob;
SET SESSION AUTHORIZATION regress_rls_bob;
COPY copy_no_rls_test FROM stdin;
1	works
\.
RESET SESSION AUTHORIZATION;
SELECT * FROM copy_no_rls_test;
DROP TABLE copy_no_rls_test;

-- T17: ON_ERROR IGNORE does not skip RLS violation
SET SESSION AUTHORIZATION regress_rls_bob;
COPY copy_rls_test FROM stdin WITH (ON_ERROR ignore);
2	2	should_still_fail
\.
RESET SESSION AUTHORIZATION;
SELECT * FROM copy_rls_test;
TRUNCATE copy_rls_test;
...
RESET SESSION AUTHORIZATION;
GRANT INSERT ON copy_rls_test TO regress_rls_exempt_user;
SET SESSION AUTHORIZATION regress_rls_exempt_user;
COPY copy_rls_test FROM stdin;
50	99	bypass_user
\.
...
-- Cleanup COPY FROM RLS test objects
RESET SESSION AUTHORIZATION;
DROP TABLE copy_rls_test;
