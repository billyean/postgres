...
/*
 * Estimate P(X < Y) for two arrays of range bounds using the approach
 * from Repas et al. 2022 (selectivity estimation of inequality joins):
 *
 ...
 *
 * Supports: <<, >>, && for all multirange/range type combinations:
 *   multirange × multirange, multirange × range, range × multirange
 *
 * These operators map to the same strict bound comparisons as their range
 * counterparts, estimated via calc_hist_join_selectivity() from bound
 * histograms.
 *
 * Multirange statistics only preserve the outermost bounds (overall lower
 * and upper) of each multirange value.  This is the same outer-bounds
 * approximation already used by multirange restriction selectivity.  For the
 * << and >> operators, outer bounds are sufficient because they depend only
 * on the overall upper and lower bounds respectively.  For &&, the estimate
 * treats each multirange as a single contiguous range spanning its outermost
 * bounds, which may overestimate overlap for sparse multiranges with large
 * internal gaps.  This is still expected to improve on the previous fixed
 * default selectivity.
 */
Datum
multirangejoinsel(PG_FUNCTION_ARGS)
{
	...
	/*
	 * Determine the underlying range type cache.  At least one side is a
	 * multirange; the other may be a multirange or a plain range.  Both sides
	 * must share the same underlying range type.
	 */
	typcache = lookup_type_cache(vardata1.vartype, TYPECACHE_MULTIRANGE_INFO);
	if (typcache->rngtype != NULL)
	{
		/* vardata1 is a multirange */
		rng_typcache = typcache->rngtype;
		if (vardata2.vartype != vardata1.vartype &&
			vardata2.vartype != rng_typcache->type_id)
			goto cleanup;
	}
	else
	{
		/* vardata1 is a range; vardata2 must be a multirange */
		typcache = lookup_type_cache(vardata2.vartype,
									 TYPECACHE_MULTIRANGE_INFO);
		if (typcache->rngtype == NULL)
			goto cleanup;
		rng_typcache = typcache->rngtype;
		if (vardata1.vartype != rng_typcache->type_id)
			goto cleanup;
	}
	...
	/* Try to get fraction of empty ranges/multiranges for the first variable */
	...
	/* Try to get fraction of empty ranges/multiranges for the second variable */
	...
	switch (operator)
	{
		...
		case OID_RANGE_OVERLAPS_MULTIRANGE_OP:
			 ...
			 *
			 * For multiranges, this uses the outermost bounds only.  Internal
			 * gaps are not represented in the available statistics, so overlap
			 * may be overestimated for sparse multiranges.  This is consistent
			 * with the existing outer-bounds approximation used by multirange
	...
	/*
	 * For the supported operators (<<, >>, &&), empty ranges/multiranges
	 * always produce false, so no empty-fraction adjustment is needed.
	 */
